...
export const eliminarFotoPlayList = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        const fotoPlayList = await AppDataSource.getRepository(ImagenPlaylist).findOne({
            where: {
                Id_imagen: Number(id)
            }
        });
        ...
        await AppDataSource.getRepository(ImagenPlaylist).remove(fotoPlayList);
