...
export const crearFotoNoticia = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    if (!id) {
        return res.status(404).json({message: "ID de la noticia es requerida."});
    }

    const noticia = await AppDataSource.getRepository(Noticia).findOne({ where: { Id_noticia: Number(id) } });

    if (!noticia) {
        return res.status(404).json({ error: "La noticia no fue encontrada." });
    }
    ...
    const { fotoNoticia } = req.files as { [fieldname: string]: UploadedFile };

    // Subir las imágenes a Imgur secuencialmente y obtener sus URLs y IDs
    const { url: fotoNoticiaUrl, id_imgur: imagenImgurId } = await subirImagen(fotoNoticia);

    // Crear el objeto foto con los datos de la foto subida
    const fotoNoticiaData = new ImagenNoticia();
    fotoNoticiaData.url = fotoNoticiaUrl;
    fotoNoticiaData.id_imgur = imagenImgurId;
    fotoNoticiaData.noticia = noticia;

    // Guardar la fotografia en la base de datos
    await AppDataSource.getRepository(ImagenNoticia).save(fotoNoticiaData);
